...
export function About() {
  return (
    <section id="about" className="scroll-mt-20 border-y border-gold/20 bg-panel py-20 md:py-28">
      <div className="mx-auto grid max-w-7xl gap-12 px-4 sm:px-6 lg:grid-cols-2">
          ...
          <ul className="mt-8 flex flex-wrap gap-3">
            {credentials.badges.map((b, i) => {
              ...
              return (
                <li
                  key={b}
                  className="inline-flex items-center gap-2 rounded-full border border-gold/50 bg-gold/10 px-4 py-2 text-sm font-semibold uppercase tracking-wider text-gold-light"
                >
        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="scorecard p-6 sm:p-8">
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-gold">{philosophy.eyebrow}</p>
            <h3 className="mt-3 text-2xl uppercase tracking-wide sm:text-3xl">{philosophy.heading}</h3>
            <ul className="mt-5 space-y-1.5 text-gold-light/90">
