  ...
  return (
    <section className="relative isolate overflow-hidden bg-panel py-20 md:py-28">
      <div className="mx-auto grid max-w-7xl items-center gap-12 px-4 sm:px-6 lg:grid-cols-2">
        <div className="relative aspect-[3/2] overflow-hidden rounded-lg border border-gold/30 shadow-[0_0_60px_-20px_var(--gold)]">
          <Image
        ...
        <div>
          ...
          <ul className="mt-6 space-y-3">
            {moreThanAScore.questions.map((q) => (
              <li key={q} className="flex items-start gap-3 text-lg">
                <HelpCircle className="mt-1 size-5 shrink-0 text-rose" aria-hidden />
          ...
          <div className="mt-8 space-y-3 border-l-2 border-gold pl-5">
            {moreThanAScore.outro.map((p, i) => (
              <p key={p} className={i === 0 ? "font-display text-xl uppercase text-gold-light" : "text-muted-foreground"}>
                {p}
              </p>
            ))}
          </div>
